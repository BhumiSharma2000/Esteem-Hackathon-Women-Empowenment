...
<?php
	...
	{	
		if($_FILES["image"]["name"] != "")
		{
			...
			$updated2 = mysqli_query($con,"UPDATE tbl_detail SET name='$name',profile_pic='$location',address='$address',dob='$dob',updated_on='CURRENT_timestamp()' WHERE login_id='$id'");
			$updated3 = mysqli_query($con,"UPDATE tbl_login SET phone_no='$newphone' WHERE login_id='$log'");
			if($updated && $updated2)
			{
				if($updated3)
				{
					        echo ("<script LANGUAGE='JavaScript'> window.alert('Succesfully Updated'); window.location.href='manageadmin.php?flag=1';</script>");    			
				}
				else
				{
